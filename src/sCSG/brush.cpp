...
constexpr float DIST_EPSILON = 0.04;
...
find_plane:
	...
	auto *p = &g_mapplanes[g_nummapplanes];
...
{
	const HullBrushFace *hbf;
	const HullBrushEdge *hbe;
	const HullBrushVertex *hbv;
	BrushFace *f;
	vec3_t normal;
	vec3_t origin;

	auto *axialbevel = (bool *)malloc(hb->numfaces * sizeof(bool));
	...
	for (f = hull0->faces; f; f = f->next)
	{
		...
		for (hbf = hb->faces; hbf < hb->faces + hb->numfaces; hbf++)
		{
			...
			{
				auto dot = DotProduct(*v, brushface.normal);
	...
	for (f = hull0->faces; f; f = f->next)
	{
		for (int i = 0; i < f->w->m_NumPoints; i++) // for each edge in f
		{
			...
			auto len = VectorLength(brushedge.delta);
...
{
	const HullShape *hs = &g_defaulthulls[hullnum];
	{ // look up the name of its hull shape in g_hullshapes[]
		...
		if (name && *name)
		{
			...
			{
				const HullShape *s = &g_hullshapes[i];
				if (!strcmp(name, s->id))
				{
					if (found)
					{
		...
		for (current_face = brush->hulls[0].faces; current_face; current_face = current_face->next)
		{
			...
			for (counter = 0; counter < (winding->m_NumPoints); counter++) // for each edge
			{
				...
				for (other_face = brush->hulls[0].faces; other_face; other_face = other_face->next)
				{
					...
					start_found = false;
					end_found = false;
					auto *other_winding = other_face->w;
				...
				auto *other_plane = other_face->plane;
...
void SortSides(BrushHull *h)
{
	...
	auto *isused = (bool *)malloc(numsides * sizeof(bool));
	hlassume(isused != nullptr, assume_NoMemory);
	auto *sorted = (int *)malloc(numsides * sizeof(int));
...
auto MakeBrushPlanes(Brush *b) -> bool
{
	...
	{
		auto *s = &g_brushsides[b->firstside + i];
		for (int j = 0; j < 3; j++)
		{
			VectorSubtract(s->planepts[j], origin, s->planepts[j]);
		}
		auto planenum = PlaneFromPoints(s->planepts[0], s->planepts[1], s->planepts[2]);
		if (planenum == -1)
		{
...
auto CheckBrushContents(const Brush *const b) -> contents_t
{
	int i;
	bool assigned = false;
	auto *s = &g_brushsides[b->firstside];
...
{
	auto *b = &g_mapbrushes[brushnum];
	auto contents = b->contents;
...
auto CreateHullBrush(const Brush *b) -> HullBrush *
{
	...
	Winding *w[MAXSIZE];
	HullBrushEdge edges[MAXSIZE];
	HullBrushVertex vertexes[MAXSIZE];
	int i;
	int j;
	int k;
	int e;
	...
	for (i = 0; i < b->numsides; i++)
	{
		...
		auto axial = PlaneTypeForNormal(normal);
		if (axial <= last_axial)
		{
	...
	if (!failed)
	{
		hb = (HullBrush *)malloc(sizeof(HullBrush));
		hlassume(hb != nullptr, assume_NoMemory);

		hb->numfaces = numplanes;
		hb->faces = (HullBrushFace *)malloc(hb->numfaces * sizeof(HullBrushFace));
		hlassume(hb->faces != nullptr, assume_NoMemory);
		for (i = 0; i < numplanes; i++)
		{
			HullBrushFace *f = &hb->faces[i];
		...
		hb->edges = (HullBrushEdge *)malloc(hb->numedges * sizeof(HullBrushEdge));
		hlassume(hb->edges != nullptr, assume_NoMemory);
		memcpy(hb->edges, edges, hb->numedges * sizeof(HullBrushEdge));

		hb->numvertexes = numvertexes;
		hb->vertexes = (HullBrushVertex *)malloc(hb->numvertexes * sizeof(HullBrushVertex));
		hlassume(hb->vertexes != nullptr, assume_NoMemory);
		memcpy(hb->vertexes, vertexes, hb->numvertexes * sizeof(HullBrushVertex));
	}
	else
	{
		hb = nullptr;
...
void CreateHullShape(int entitynum, bool disabled, const char *id, int defaulthulls)
{
	...
	auto *entity = &g_entities[entitynum];
	...
	{
		Brush *b = &g_mapbrushes[entity->firstbrush + i];
	...
	if (hs->numbrushes >= 2)
	{
		Brush *b = &g_mapbrushes[entity->firstbrush];
	...
	for (int h = 0; h < NUM_HULLS; h++)
	{
		if (defaulthulls & (1 << h))
		{
			HullShape *target = &g_defaulthulls[h];
