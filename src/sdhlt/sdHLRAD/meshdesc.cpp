...
auto CMeshDesc ::PlaneEqual(const mplane_t *p0, const mplane_t *p1) -> bool
{
	...
	if (-PLANE_DIST_EPSILON < (t = p0->dist - p1->dist) && t < PLANE_DIST_EPSILON && -PLANE_NORMAL_EPSILON < (t = p0->normal[0] - p1->normal[0]) && t < PLANE_NORMAL_EPSILON && -PLANE_NORMAL_EPSILON < (t = p0->normal[1] - p1->normal[1]) && t < PLANE_NORMAL_EPSILON && -PLANE_NORMAL_EPSILON < (t = p0->normal[2] - p1->normal[2]) && t < PLANE_NORMAL_EPSILON)
		return true;
...
auto CMeshDesc ::StudioConstructMesh(model_t *pModel) -> bool
{
	...
	for (i = 0; i < phdr->numbones; i++)
	{
		...
		if (pbone[i].parent == -1)
			...
			ConcatTransforms(bonetransform[pbone[i].parent], bonematrix, bonetransform[i]);
	}
